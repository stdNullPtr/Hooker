...
DWORD WINAPI StartRoutine(LPVOID lpParam)
{
	...
	const auto* const pWsaSendToFuncAddr{ (uintptr_t*)wsaSendToFuncAddr };
	const auto* const pLogShellcodeAddr{ (uintptr_t*)Hooker::LogShellcode };
	const auto* const pWsaSendToRetAddr{ (uintptr_t*)((const char* const)pWsaSendToFuncAddr + 0xF) };
	void* const pTrampolineBlock{ VirtualAlloc(nullptr, 0xFF, MEM_COMMIT, PAGE_EXECUTE_READWRITE) };

	memcpy(&Hooker::WsaSendTo::trampolineShellcode[27], &pLogShellcodeAddr, sizeof pLogShellcodeAddr);
	...
	std::cout << XorStr("WSASendTo() addr: ") << std::hex << std::uppercase << pWsaSendToFuncAddr << '\n';
